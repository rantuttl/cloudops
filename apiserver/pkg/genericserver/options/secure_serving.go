...
func (s *SecureServingOptions) AddDeprecatedFlags(fs *pflag.FlagSet) {
	// Add any deprecated flags in the future
}
...
func (s *SecureServingOptions) ApplyTo(c *genericserver.Config) error {
	...
	// FIXME (rantuttl): What does this do, and will we need it. Cleanup!!
	// create self-signed cert+key with the fake genericserver.LoopbackClientServerNameOverride and
	// let the server return it when the loopback client connects.
	/*
	certPem, keyPem, err := certutil.GenerateSelfSignedCertKey(genericserver.LoopbackClientServerNameOverride, nil, nil)
	if err != nil {
	...
	default:
		...
		c.SecureServingInfo.SNICerts[genericserver.LoopbackClientServerNameOverride] = &tlsCert
	}
	*/
...
func (s *SecureServingOptions) applyServingInfoTo(c *genericserver.Config) error {
	if s.BindPort <= 0 {
		return nil
	}

	secureServingInfo := &genericserver.SecureServingInfo{
	...
	// FIXME (rantuttl): Remove if we don't need SNI...what is it? it's not in the SercureServingInfo at this time
	// load SNI certs
	/*
	namedTLSCerts := make([]genericserver.NamedTLSCert, 0, len(s.SNICertKeys))
	for _, nck := range s.SNICertKeys {
		tlsCert, err := tls.LoadX509KeyPair(nck.CertFile, nck.KeyFile)
		namedTLSCerts = append(namedTLSCerts, genericserver.NamedTLSCert{
			TLSCert: tlsCert,
			Names:   nck.Names,
		})
		if err != nil {
	...
	var err error
	secureServingInfo.SNICerts, err = genericserver.GetNamedCertificateMap(namedTLSCerts)
	if err != nil {
		return err
	}
	*/
